...
export default function ClientDashboardPage() {
 ...
 {/* Tab Navigation */}
 <div className="flex items-center gap-1 bg-card border border-border rounded-xl p-1 mb-8 w-fit">
 {tabs.map((tab) => (
 <button
 key={tab.id}
 ...
 <p className="text-2xl font-bold text-foreground">₹{(stats?.totalPaid || 0).toLocaleString()}</p>
 </div>
 <div className="bg-card border border-border rounded-2xl p-5">
 <div className="flex items-center gap-3 mb-3">
 ...
 <p className="text-2xl font-bold text-foreground">₹{(stats?.totalPending || 0).toLocaleString()}</p>
 </div>
 <div className="bg-card border border-border rounded-2xl p-5">
 <div className="flex items-center gap-3 mb-3">
 ...
 <p className="text-2xl font-bold text-foreground">{stats?.pendingInvoices || 0}</p>
 </div>
 </div>
 ...
 <p className="text-sm text-muted-foreground">All invoices are cleared!</p>
 </div>
 ) : (
 <div className="space-y-3">
 {payments
 ...
 >
 <div className="min-w-0">
 <p className="text-sm font-medium text-foreground">#{payment.invoiceNumber}</p>
 <p className="text-xs text-muted-foreground mt-0.5">
 ...
 <div
 key={project._id}
 className="bg-card border border-border rounded-2xl p-5 hover:shadow-lg hover:shadow-primary-500/5 transition-shadow"
 >
 <div className="flex items-start justify-between mb-3">
 <h4 className="font-semibold text-foreground text-sm leading-snug">{project.projectName}</h4>
 <span className={`text-xs font-medium px-2.5 py-1 rounded-full whitespace-nowrap ${getStatusColor(project.status)}`}>
 {project.status}
 </span>
 </div>
 ...
 <span key={tech} className="text-[10px] px-2 py-0.5 bg-muted text-muted-foreground rounded-md font-medium">
 {tech}
 </span>
 ))}
 ...
 <th className="text-left py-3.5 px-5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Invoice</th>
 <th className="text-left py-3.5 px-5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Project</th>
 <th className="text-left py-3.5 px-5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Amount</th>
 ...
 <th className="text-right py-3.5 px-5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Action</th>
 </tr>
 </thead>
