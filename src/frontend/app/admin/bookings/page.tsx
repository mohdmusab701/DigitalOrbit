    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 ml-10">Manage Discovery Calls and Appointments.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-xl text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-primary-500"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => { setStatusFilter(e.target.value); }}
          className="px-3 py-2.5 bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-xl text-sm text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-primary-500"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-slate-200 dark:border-dark-border rounded-2xl bg-white dark:bg-dark-card">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {bookings.map((booking) => (
            <motion.div
              ...
              className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col"
            >
              {/* Card Header */}
              <div className="p-5 border-b border-slate-100 dark:border-dark-border flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white">{booking.name}</h3>
              ...
              <div className="p-5 flex-1 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-white">{format(parseISO(booking.meetingDate), "EEEE, MMM dd")}</p>
              ...
              {/* Card Footer */}
              <div className="p-4 bg-slate-50 dark:bg-dark-bg border-t border-slate-100 dark:border-dark-border flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-sm font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 px-3 py-1.5 bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-lg shadow-sm"
                >
                  Manage
                </button>
              </div>
        ...
        {selectedBooking && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden"
            >
              <div className="px-6 py-4 border-b border-slate-200 dark:border-dark-border flex items-center justify-between">
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Manage Booking</h2>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <strong className="text-slate-900 dark:text-white">{selectedBooking.name}</strong>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <strong className="text-slate-900 dark:text-white">{format(parseISO(selectedBooking.meetingDate), "MMM dd, yyyy")} at {selectedBooking.meetingTime}</strong>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-dark-border">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Change Status</label>
                    <select
                      value={editStatus}
                      onChange={(e) => setEditStatus(e.target.value)}
                      className="w-full px-3 py-2 bg-white dark:bg-dark-bg border border-slate-200 dark:border-dark-border rounded-xl text-sm"
                    >
                      <option value="pending">Pending</option>
                      <option value="approved">Approved</option>
                  ...
                  {editStatus === "approved" && selectedBooking.meetingType !== "Phone" && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Meeting Link (sent via email)</label>
                      <input
                        type="url"
